...
public class AllMethodsOfObjectClass {
    ...
    public static void main(String[] args) throws InterruptedException {
        UsingWaitAndNotifyClass service = new UsingWaitAndNotifyClass();
        waitMethod(service);
        notifyMethod(service);
        TimeWaitMethod method = new TimeWaitMethod(new Object());
        Thread.sleep(3000L);
        method.threadTimeWait();
        method.threadTimeNotify();
    }
    ...
    public static void waitMethod(UsingWaitAndNotifyClass service) {
        new ThreadWait("ThreadWait", service).start();
    }

    public static void notifyMethod(UsingWaitAndNotifyClass service) {
        new ThreadNotify("ThreadNotify", service).start();
    }
    ...
    static class TimeWaitMethod {
        private final Object object;

        public TimeWaitMethod(Object object) {
            this.object = object;
        }

        public void threadTimeWait() throws InterruptedException {
            synchronized (object) {
                System.out.format("开始等待. 当前执行的线程是:%s, 现在的时间是%d.\n", Thread.currentThread().getName(), System.currentTimeMillis());
                object.wait(4000L);
                System.out.format("结束等待. 当前执行的线程是:%s, 现在的时间是%d.\n", Thread.currentThread().getName(), System.currentTimeMillis());
            }
        }

        public void threadTimeNotify(){
            synchronized (object){
                System.out.format("开始唤醒. 当前执行的线程是:%s, 现在的时间是%d.\n", Thread.currentThread().getName(), System.currentTimeMillis());
                object.notify();
                System.out.format("结束唤醒. 当前执行的线程是:%s, 现在的时间是%d.\n", Thread.currentThread().getName(), System.currentTimeMillis());
            }
        }
    }
    ...
    private static class UsingWaitAndNotifyClass {
        ...
        /**
         * 使用{@link Object#wait()}阻塞线程.等待notify操作将其唤醒
         *
         * @throws InterruptedException exception 出现中断异常
         */
        public void usingWait() throws InterruptedException {
            synchronized (mutex) {
                System.out.format("开始等待. 当前执行的线程是:%s, 现在的时间是%d.\n", Thread.currentThread().getName(), System.currentTimeMillis());
                mutex.wait();
                System.out.format("结束等待. 当前执行的线程是:%s, 现在的时间是%d.\n", Thread.currentThread().getName(), System.currentTimeMillis());
        ...
        /**
         * 通过{@link Object#notify()}方法将等待的线程进行唤醒.
         */
        public void usingNotify() {
            synchronized (mutex) {
                System.out.format("开始唤醒. 当前执行的线程是:%s, 现在的时间是%d.\n", Thread.currentThread().getName(), System.currentTimeMillis());
                mutex.notify();
                System.out.format("结束唤醒. 当前执行的线程是:%s, 现在的时间是%d.\n", Thread.currentThread().getName(), System.currentTimeMillis());
    ...
    static class ThreadWait extends Thread {
        ...
        private final UsingWaitAndNotifyClass service;

        public ThreadWait(String name, UsingWaitAndNotifyClass service) {
            super(name);
    ...
    static class ThreadNotify extends Thread {
        ...
        public ThreadNotify(String name, UsingWaitAndNotifyClass service) {
            super(name);
            this.service = service;
        }
